...
| FR-02 | `run <id>` / `run --all` — `subprocess.run(shlex.split, …, timeout)` no `shell=True`; state machine `pending → running → done\|failure\|timeout\|blocked`; result fields incl. `stdout_tail` / `stderr_tail` last 2000 chars; `ThreadPoolExecutor` + shared `threading.Lock` for `--all` | §3 FR-02 | AC-FR-02.a, .b, .c, .d, .e | test_fr02_a, test_fr02_b, test_fr02_c, test_fr02_d, test_fr02_e | `03-development/src/taskq_plus/service/executor.py` | `03-development/tests/test_executor.py` | DRAFT |
| FR-03 | Retry on terminal-state exit ≤ `TASKQ_RETRY_LIMIT` with exponential backoff `TASKQ_BACKOFF_BASE × 2^n` (sleep injectable); circuit breaker `CLOSED → OPEN` at `TASKQ_BREAKER_THRESHOLD` consecutive terminal exits; `OPEN` rejects with exit 3 + stderr `breaker open`; `OPEN → HALF_OPEN → CLOSED` after `TASKQ_BREAKER_COOLDOWN`; atomic `breaker.json` | §3 FR-03 | AC-FR-03.a, .b, .c, .d | test_fr03_a, test_fr03_b, test_fr03_c, test_fr03_d | `03-development/src/taskq_plus/service/breaker.py` | `03-development/tests/test_breaker.py` | DRAFT |
